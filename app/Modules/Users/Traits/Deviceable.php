...
trait Deviceable
{
    ...
    {
        if ($this->getDeviceToken($deviceOptions)) {
            return;
        }
    ...
    /**
     * Get user devices ids for firebase Android
     *
     * @return array
     */
    public function getAndroidFirebaseIds(): array
    {
        return collect($this->devices)->where('type', 'android')->pluck('token')->toArray();
    }
     ...
     * @return array
     */
    public function getIOSFirebaseIds(): array
    {
        return collect($this->devices)->where('type', '!=', 'android')->pluck('token')->toArray();
    }
}
